/* globals setupAvatar:true, moveAvatar:true, resetAvatar:true */
...
{
  if (l.pulsesize<0.01) { return 0; }
  pulsePos=(timeStamp%l.pulsetime)*2.0/l.pulsetime-1;
...
function pulseFactor(l,timeStamp)
{
  ...
  pulsePos=(timeStamp%l.pulsetime)*2.0/l.pulsetime;
  if (pulsePos<1) {
...
function touchEvent(x,y,down)
{
  if (down) {
    ...
    if (Math.abs(xdif)>Math.abs(ydif)) { //move sideways
      ...
      if (xdif<-20) { avatar.command=2; }
    } else {  //move across
      if (ydif>20) { avatar.command=1; }
      if (ydif<-20) { avatar.command=3; }
    }
    if (avatar.command!==0) {
      lastMouseX=x;
      lastMouseY=y;
...
function resetBoard()
{
  levid='level'+board.currentLevel;
  level=document.getElementById(levid);
  ...
  for(var i = 0; i < bits.length; i++) {
    ...
    for(var j = 0; j < children.length; j++)  {
      ...
      if (ch.classList.contains('refuse')) {
        ob={left: ch.offsetLeft, right: ch.offsetLeft+ch.offsetWidth, type: 'r'};
        l.members.push(ob);
      }
      if (ch.classList.contains('kill')) {
        ob={left: ch.offsetLeft, right: ch.offsetLeft+ch.offsetWidth, type: 'k'};
        l.members.push(ob);
      }
      if (ch.classList.contains('target')) {
        ob={left: ch.offsetLeft, right: ch.offsetLeft+ch.offsetWidth, type: 't'};
        l.members.push(ob);
      }
    }
...
function refreshBoardInfo()
{
  if (board.info.classList.contains('infopanel-inplay')) {
    if (!board.isplaying) {
      board.info.classList.remove('infopanel-inplay');
    } else {
      if (board.isplaying) {
        board.info.classList.add('infopanel-inplay');
      }
    }
  }
  ...
  if (board.classList.contains('gameframe-inplay')) {
    if (!board.isplaying) {
      board.classList.remove('gameframe-inplay');
    }
  } else {
    if (board.isplaying) {
      board.classList.add('gameframe-inplay');
    }
  }
  ...
  case 0:
    board.infostatus.innerHTML='';
    break;
  case 1:
    board.infostatus.innerHTML='Game Paused';
    break;
  case 2:
    board.infostatus.innerHTML='You Won';
    break;
  case 3:
    board.infostatus.innerHTML='You Died';
    break;
  }
